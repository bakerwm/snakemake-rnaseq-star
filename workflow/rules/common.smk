...
from snakemake.utils import validate, _load_configfile
...
def get_samples():
    ...
    validate(
        samples, schema=workflow.source_path("../schemas/samples.schema.yaml")
    )
    return samples
...
def get_units():
    ...
    validate(
        units, schema=workflow.source_path("../schemas/units.schema.yaml")
    )
    return units
